...
class SSDInputEncoder:
    ...
    def __init__(self,
        ...
        for i in range(len(self.predictor_sizes)):#Сказано что возвращает 4D , почему на прием стоит 5 переменных?????
    ...
    def __call__(self, ground_truth_labels, diagnostics=False):
        ...
        for i in range(batch_size): #For each batch item
            ...
            if ground_truth_labels[i].size == 0:# If there is no ground truth for this batch item, there is nothing to match.
                    continue
